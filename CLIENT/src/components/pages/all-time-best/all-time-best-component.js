...
import {GET_ALL_TIME_BEST_COINS } from '../../../services/graphql'
...
export default function AtbCoins() {
    ...
    const {loading, data, fetchMore, networkStatus} = useQuery(GET_ALL_TIME_BEST_COINS, { 
    ...
    const onLoadMore = () => {
      try {
        fetchMore({
            variables: {
                offset: data.AllTimeBest.length
            }
        });
      } catch (err) {
            return err
    ...
    return (
        <div className="coin-page">
            ...
            {networkStatus === 1  ? <div style={{width: '100%', minHeight: 450, textAlign: 'center'}}><CircularProgress/></div> :
                ...
                data?.AllTimeBest.length === 0 ? <p style={{color: '#5c5c5c', fontSize: 14, textAlign: 'center'}}>No listed coins found.&nbsp; 
